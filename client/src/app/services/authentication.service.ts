...
export class AuthenticationService {
    ...
    login(username: string, password: string): Observable<boolean> {
      console.log(JSON.stringify({ logemail: username, logpassword: password }));
        ...
        return this.http.post(this.utilservice.getLoginUrl(), { logemail: username, logpassword: password })
    ...
    register(email: string, username: string, password: string, passwordConf: string): Observable<boolean> {
      console.log(JSON.stringify({ logemail: username, logpassword: password }));
        ...
        return this.http.post(this.utilservice.getRegisterUrl(), {email:email, username: username, password: password, passwordConf : passwordConf })
            .map((response: Response) => {
                // login successful if there's a jwt token in the response
